...
func TradeUpCards(ownedCardIDs []string, ownerID string) ([]domain.CardData, error) {
	ownedCards := make([]domain.OwnedCard, 0)
	weightBySet := make(map[string]int, 0)
	weightByRarity := make(map[string]int, 0)
	for _, ownedCardId := range ownedCardIDs {
		ownedCard, err := db.GetOwnedCardById(ownedCardId)
		if err != nil {
		...
		weightBySet[ownedCard.CardData.SetCode] += 1
		weightByRarity[string(ownedCard.CardData.Rarity)] += 1
		ownedCards = append(ownedCards, ownedCard)
	}
	...
	boosterPack := domain.BoosterPack{
		SetCode:     "C-TRD",
		Name:        "Tradeup Pack",
		Description: "Autogenerated Tradeup Pack",
		CardCount:   5,
		Filter:      "",
		Slots: []struct {
	...
	err = db.TradeUpCards(ownedCards, cardsToAdd, ownerID)
	return cardsToAdd, err
}
